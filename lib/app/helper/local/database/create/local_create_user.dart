...
Future<List> localCreateUser(
  ...
  return await run(() async {
    ...
    if ((Utils.getKey(data, ["users"], []) as List).firstWhereOrNull(
        ...
        null) {
      List updatedList = Utils.addItemToList(data, [
        "users"
      ], {
        "name": name,
        "email": email,
      ...
      print("updated list: " + updatedList.toString());
      data = await localUpdateKey(
          location: [], updateData: {"users": updatedList});
      await localWriteData(data);
    }

    return Utils.getKey(data, ["users"], []);
  });
}
